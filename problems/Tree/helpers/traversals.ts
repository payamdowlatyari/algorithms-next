 ...
 */
export function preOrderTraversal(root: TreeNode | null): Array<any> {
  if (!root) return [];
 ...
 * Space complexity: O(n) where n is the number of nodes in the tree
 */
export function inOrderTraversal(root: TreeNode | null): Array<any> {
  if (!root) return [];
 ...
 * Space complexity: O(n) where n is the number of nodes in the tree
 */
export function postOrderTraversal(root: TreeNode | null): Array<any> {
  if (!root) return [];
 ...
 * Space complexity: O(n) where n is the number of nodes in the tree
 */
export function breadthFirstTraversal(tree: TreeNode | null): Array<any> {
  if (!tree) return [];
  let queue = [tree];
  let result = [];
 ...
 * Space complexity: O(n) where n is the number of nodes in the tree
 */
export function depthFirstTraversal(tree: TreeNode | null): Array<any> {
  if (!tree) return [];
  let stack = [tree];
  let result = [];
 ...
 */
export function drawTree(tree: TreeNode | null, len: number = 10) {
  const drawMatrix = Array.from({ length: len }, () => Array(len).fill(' '));

  /**
   * Recursively draws a subtree in the 2D grid.
   *
   * @param {TreeNode} node - The root node of the subtree to draw.
   * @param {number} x - The x-coordinate of the node in the grid.
   * @param {number} y - The y-coordinate of the node in the grid.
   */
  function drawNode(node: TreeNode | null, x: number, y: number) {
    if (
      !node ||
      y < 0 ||
      y >= drawMatrix.length ||
