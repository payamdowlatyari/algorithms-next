...
export class Heap {
  array: any[];
  comparator: (i1: string | number, i2: string | number) => number;
  /**
   ...
   */
  constructor(
    comparator: (a: number, b: number) => number = (a: number, b: number) =>
      a - b,
  ) {
    this.array = [];
    /**
     ...
     */
    this.comparator = (i1: string | number, i2: string | number): number => {
      const value = comparator(
        this.array[i1 as number],
        this.array[i2 as number],
      );
      if (Number.isNaN(value))
        throw new Error(
          `Comparator should evaluate to a number. Got ${value} when comparing ${
            this.array[i1 as number]
          } with ${this.array[i2 as number]}`,
        );
      return value;
    };
  }
