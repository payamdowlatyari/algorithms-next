 ...
 */
export function linkedListToArray(head: ListNode | null): Array<number> {
  const result = [];
 ...
 */
export function arrayToLinkedList(arr: Array<number>): ListNode | null {
 ...
 * Space Complexity: O(n)
 */
export function printLinkedList(head: ListNode): void {
  let current: ListNode | null = head;
  const values = [];
  while (current) {
